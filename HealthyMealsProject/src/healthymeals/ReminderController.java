...
public class ReminderController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        lvalarm.getSelectionModel().selectedItemProperty().addListener(e-> {
                ...
                if(index>=0){
                    String[] item = obalarm.get(index).split("at");
                    int in = listTimer.get(index).getTitle().length();
                    String otitle = item [0].substring(0, in);
                    String _date=item [0].substring( in);
                    String _time = item [1];
                    title.setText(otitle);
                    date.getEditor().setText(_date);
                    time.getEditor().setText( _time); 
                }
            });
